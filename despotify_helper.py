#!/usr/bin/env python3
"""Descarga en MP3 una playlist exportada de Spotify (formato Exportify CSV).

Uso:
    python3 despotify_helper.py <playlist.csv>

Estructura de salida: ~/Music/despotify_helper/<Artista>/<Album>/<Cancion>.mp3
Resumible: si el .mp3 ya existe, salta.
Requiere: yt-dlp y ffmpeg en el PATH (brew install yt-dlp ffmpeg).
"""
...
def main(csv_path: Path) -> int:
    ...
    for tool in ("yt-dlp", "ffmpeg"):
        if not shutil.which(tool):
            print(f"{tool} no está en el PATH (brew install {tool})", file=sys.stderr)
            return 1
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Uso: python3 despotify_helper.py <playlist.csv>", file=sys.stderr)
        sys.exit(1)
